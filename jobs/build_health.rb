...
def api_functions
  return {
    'Android' => lambda { |build_id| get_jenkins_build_health 'android', build_id},
    'Espresso' => lambda { |build_id| get_jenkins_build_health 'espresso', build_id},
    'iOS' => lambda { |build_id| get_jenkins_build_health 'ios', build_id},
    'Services' => lambda { |build_id| get_jenkins_build_health 'services', build_id},
    'Web' => lambda { |build_id| get_jenkins_build_health 'web', build_id}
  }
end
...
def get_jenkins_build_health(server, build_id)
  baseUrl = case server
    when "espresso" then Builds::BUILD_CONFIG['espressoBaseUrl']
    when "android" then Builds::BUILD_CONFIG['androidBaseUrl']
    when "ios" then Builds::BUILD_CONFIG['iosBaseUrl']
    when "services" then Builds::BUILD_CONFIG['servicesBaseUrl']
    when "web" then Builds::BUILD_CONFIG['webBaseUrl']
  end
  # url = "#{baseUrl}/job/#{build_id}/api/json?tree=builds[status,timestamp,id,result,duration,url,fullDisplayName,actions[parameters[name,value],buildsByBranchName]]"
